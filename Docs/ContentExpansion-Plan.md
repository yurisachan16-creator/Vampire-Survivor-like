...
| 精灵名 | 位置 | 外观 | 建议用途 | 优先级 |
|---|---|---|---|---|
| `rpgItems_16` | (0,2) | 橙子 | **经验掉落物视觉变体** — 大经验球 | ★☆☆ |
| `rpgItems_17` | (1,2) | 蒜头 | **大蒜被动替代图标**（若不用蘑菇） | ★☆☆ |
| `rpgItems_18` | (2,2) | 葡萄 | **新掉落物：葡萄** — 回复 2 HP | ★☆☆ |
| `rpgItems_19` | (3,2) | 柠檬 | **新掉落物：柠檬** — 5 秒伤害提升 buff | ★★☆ |
| `rpgItems_20` | (4,2) | 木盾 | **新被动：格挡（Block）** — 概率完全抵消伤害 | ★★☆ |
| `rpgItems_21` | (5,2) | 铜盾 | **格挡被动 Lv2 图标** | ★☆☆ |
| `rpgItems_22` | (6,2) | 铁盾 | **格挡被动 Lv3 图标** | ★☆☆ |
| `rpgItems_23` | (7,2) | 水晶盾 | **格挡进化后图标** | ★☆☆ |

### 第 3 行（y=64） —— 水果 & 杂项

| 精灵名 | 位置 | 外观 | 建议用途 | 优先级 |
|---|---|---|---|---|
| `Exp` | (0,3) | 红苹果 | **已使用** — 经验掉落物（当前名称） | — |
| `rpgItems_25` | (1,3) | 绿苹果 | **新掉落物：绿苹果** — 小量经验 buff | ★☆☆ |
| `rpgItems_26` | (2,3) | 蓝莓 | **备用掉落物素材** | ☆☆☆ |
| `rpgItems_27` | (3,3) | 柠檬（黄） | **备用 buff 掉落素材** | ☆☆☆ |
| `rpgItems_28` | (4,3) | 棕色盾（心形） | **新被动：生命力（Vitality）** — 增加最大 HP | ★★★ |
| `rpgItems_29` | (5,3) | 战斧盾 | **备用装备素材** | ☆☆☆ |
| `rpgItems_30` | (6,3) | 铁盾（小） | **备用装备素材** | ☆☆☆ |
| `rpgItems_31` | (7,3) | 冰晶盾 | **备用装备素材** | ☆☆☆ |

### 第 4 行（y=48） —— 肉 & 斧头/剑

| 精灵名 | 位置 | 外观 | 建议用途 | 优先级 |
|---|---|---|---|---|
| `rpgItems_32` | (0,4) | 烤肉 | **新掉落物：烤肉** — 回复 3 HP（稀有掉落） | ★★★ |
| `rpgItems_33` | (1,4) | 红心 | **HP UI 显示素材** — 替换/增强 HP 心形图标 | ★★☆ |
| `rpgItems_34` | (2,4) | 香蕉/黄金果 | **新掉落物：黄金果** — 同时获得经验+金币 | ★☆☆ |
| `rpgItems_35` | (3,4) | 白剑 | **新武器：圣剑（Holy Sword）** — 直线穿透光束 | ★★★ |
| `rpgItems_36` | (4,4) | 暗剑 | **圣剑进化后：暗影剑（Shadow Blade）** | ★★☆ |
| `rpgItems_37` | (5,4) | 战斧 | **新武器：斧头（Axe）** — 抛物线飞斧（激活已有 SimpleAxe） | ★★★ |
| `rpgItems_38` | (6,4) | 银斧 | **斧头进化后图标** | ★☆☆ |
| `rpgItems_39` | (7,4) | 白斧 | **备用武器素材** | ☆☆☆ |
...
| `rpgItems_41` | (1,5) | 金币 | **金币掉落物图标优化** — 替换现有金币精灵 | ★★☆ |
| `rpgItems_42` | (2,5) | 白色药瓶 | **新被动：圣水（Holy Water）** — 脚下留毒圈 | ★★☆ |
| `rpgItems_43` | (3,5) | 铜奖章 | **成就图标素材** | ☆☆☆ |
| `rpgItems_44` | (4,5) | 流星锤 | **新武器：流星锤（Flail）** — 环绕+甩出攻击 | ★★☆ |
| `rpgItems_45` | (5,5) | 战锤 | **流星锤进化后：战锤（War Hammer）** | ★☆☆ |
| `rpgItems_46` | (6,5) | 木杖 | **新武器：魔杖（Magic Wand）** — 自动瞄准弹幕 | ★★★ |
| `rpgItems_47` | (7,5) | 白杖 | **魔杖进化后：圣杖（Holy Staff）** | ★★☆ |

### 第 6 行（y=16） —— 远程武器

| 精灵名 | 位置 | 外观 | 建议用途 | 优先级 |
|---|---|---|---|---|
| `rpgItems_48` | (0,6) | 炸弹（燃烧） | **炸弹武器图标优化** — 替换现有 bomb_icon | ★★☆ |
| `rpgItems_49` | (1,6) | 蓝斧/飞镖 | **新武器：飞镖（Boomerang）** — 飞出后返回 | ★★★ |
| `rpgItems_50` | (2,6) | 弩 | **飞刀进化后图标：连弩（Crossbow）** | ★★☆ |
| `rpgItems_51` | (3,6) | 魔法书 | **新被动：空日之书（Empty Tome）** — 全局冷却缩减 | ★★★ |
| `rpgItems_52` | (4,6) | 红弓 | **新武器：弓箭（Bow）** — 长距离穿透射击 | ★★☆ |
| `rpgItems_53` | (5,6) | 白弓 | **弓箭进化后：精灵弓（Elven Bow）** | ★☆☆ |
| `rpgItems_54` | (6,6) | 长矛 | **新武器：长矛（Spear）** — 面前直线突刺 | ★★☆ |
| `rpgItems_55` | (7,6) | 骑枪 | **长矛进化后：龙骑枪（Lance）** | ★☆☆ |
...
| `rpgItems_57` | (1,7) | 翡翠戒指 | **新被动：幸运（Luck）** — 增加掉落品质 | ★★☆ |
| `rpgItems_58` | (2,7) | 绿色护符 | **新被动：吸引器（Attractorb）** — 增加拾取范围 | ★★☆ |
| `rpgItems_59` | (3,7) | 绿色卷轴 | **备用素材** — 可用于卡牌/技能书系统 | ★☆☆ |
| `rpgItems_60` | (4,7) | 飞箭 | **弓箭弹体精灵** — 箭矢飞行物 | ★★☆ |
| `rpgItems_61` | (5,7) | 骨头 | **新武器弹体：骨头（Bone）** — 反弹型投射物 | ★★☆ |
| `rpgItems_62` | (6,7) | 细剑 | **剑武器精灵优化** — 更精致的剑斩视觉 | ★☆☆ |
| `rpgItems_63` | (7,7) | 暗战斧 | **斧头弹体精灵** — 飞行斧投射物 | ★★☆ |
...
|---|---|---|---|---|---|---|---|---|
| 6 | **斧头** | `simple_axe` | 抛物线 | `rpgItems_37` / 弹体 `rpgItems_63` | 向上抛出飞斧，受重力影响沿抛物线落下，穿透所有敌人 | 伤害 8, 间隔 1.2s, 数量 1 | 烛台 `candelabrador` | **死亡旋风 (Death Spiral)** — 360° 大范围旋转斧 |
