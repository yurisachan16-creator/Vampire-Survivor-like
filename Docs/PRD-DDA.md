...
当前项目的 `EnemyGenerator` 实现为**时间轴频道系统**，并不存在独立的“波次计数调度”或显式 `Elite` phase。DDA 因此采用 **Boss 时间轴偏移** 方案：只对 `Phase == Boss` 的频道在运行时注入开始/结束时间偏移量，提前或延后 Boss 出场窗口。

| 压力状态 | Boss 时间轴偏移 |
|----------|----------------|
| `Easy` | 最多提前 **N 分钟**（见第 9 节） |
| `Stressed` | 最多延后 **N 分钟**（见第 9 节） |

> 当前代码中没有独立的 Elite phase。名称上带“精英”的小怪频道只通过生成密度与属性倍率**间接受到** DDA 影响，不引入基于名称的脆弱识别规则。
...
| Boss 时间轴偏移 | 注入 `float RuntimeBossTimeOffsetSeconds` 至 Boss 频道启停判断逻辑 | 待实现 |
...
| Boss 最大提前分钟数 | 3 分钟 | 1 分钟 | 6 分钟 | `Easy` 状态下 Boss 最多提前的时间 |
| Boss 最大延后分钟数 | 5 分钟 | 1 分钟 | 10 分钟 | `Stressed` 状态下 Boss 最多推迟的时间 |
...
| Q4 | Boss 出场时机调整方式？ | ✅ **基于时间轴偏移**，沿用 `EnemyGenerator` 现有 `StartTimeSec / EndTimeSec` 频道系统 | 第 4.4 节、第 8.2 节 |
...
- [ ] `Easy` 状态下 Boss 时间轴提前不超过配置的最大提前分钟数
- [ ] `Stressed` 状态下 Boss 时间轴延后不超过配置的最大延后分钟数
